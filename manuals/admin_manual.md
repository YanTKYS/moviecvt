...
| 標準プレビューで再生できない | 「代替プレビューで開き直しますか？」ダイアログで「はい」を選ぶ（v0.3.1） |
| 代替プレビューでも再生できない | 事前変換ダイアログを試す。解決しない場合は DX 担当に相談 |

#### 代替プレビュー方式（v0.3.1）の技術詳細

代替プレビューは WPF MediaElement を ElementHost 経由で WinForms に埋め込んで実現しています。

| 項目 | 内容 |
|------|------|
| 技術 | `System.Windows.Controls.MediaElement` + `System.Windows.Forms.Integration.ElementHost` |
| 依存 | .NET WPF ランタイム・端末にインストールされているコーデック |
| 動作環境 | .NET 8 Desktop Runtime を含む環境（セルフコンテインド配布時は内包） |
| 再生可能形式 | 端末のコーデック環境に依存。MP4/H.264/AAC は通常再生可能 |
| 制約 | 端末によってコーデックの対応範囲が異なる場合がある |

**切り替えログの確認:**
- `[プレビュー] プレビュー方式を切り替えました: 代替` → 代替プレビューに切り替え済み
- `[代替プレイヤー 読み込み成功]` → WPF MediaElement で読み込み成功
- `[代替プレイヤー エラー] {メッセージ}` → WPF MediaElement での読み込み失敗
...
**両方式とも失敗した場合（v0.3.1以降）:** 「標準のプレビューで再生できませんでした。代替プレビューで開き直しますか？」ダイアログが表示されます。「はい」を選択すると代替プレビュー（WPF MediaElement）で再読み込みします。代替プレビューでも失敗した場合に事前変換のダイアログが表示されます。
