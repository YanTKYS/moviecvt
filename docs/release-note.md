## v0.4.11
### Title
動画簡易変換ツール v0.4.11 — 代替プレビューのサムネイル生成診断強化

### Note
代替プレビューでサムネイルが表示されない問題が発生したため、原因特定のための改善を行いました。ffmpeg が失敗した場合に従来は無音で黒のままでしたが、エラー内容をログに出力しステータス表示するよう変更しました。また、引数の渡し方を `Arguments` 文字列から `ArgumentList` に切り替え、日本語ファイル名など Unicode パスでの引数エスケープ問題を回避しました。

### 変更内容

#### 改善: 代替プレビューのサムネイル生成（FallbackPreviewPanel）

| 項目 | 内容 |
|------|------|
| `LogMessage` イベント追加 | サムネイル失敗時のエラーをメインのログ欄に表示 |
| ffmpegPath 未発見時のフィードバック | ステータスラベルに「⚠ ffmpeg.exe が見つかりません」を赤字で表示 |
| サムネイル生成失敗時のフィードバック | ステータスラベルに「⚠ サムネイルを生成できませんでした」、ログに終了コードと ffmpeg stderr 最終行を出力 |
| `ArgumentList` 切り替え | `Arguments` 文字列の手動引用符から `ArgumentList` へ変更。日本語ファイル名など Unicode パスの引数エスケープ問題を回避 |
| `StandardOutputEncoding = UTF8` 追加 | `FfmpegRunner` と統一 |
| タイムアウト 5秒 → 8秒 | 大容量ファイルへの対応 |
| 例外のログ出力 | 従来は `catch { }` で無音に飲み込んでいた例外を `LogMessage` でログに出力 |

#### バージョン更新

| ファイル | 変更 |
|---------|------|
| MainForm.cs | タイトル v0.4.11 |
| MovieConverter.csproj | バージョン 0.4.11.0 / v0.4.11 |
...
### Note
WMPプレビュー（検証実装）の本番環境実機確認に向けた記録テンプレートを development_report.md に追加しました。実際の確認作業はユーザー側の Windows 端末で行います。コード・動作に変更はありません。
...
| 項目 | 内容 |
|------|------|
| development_report.md | v0.4.9 実機確認テンプレートセクション追加（確認環境・確認結果・比較・今後の扱い判断・残課題） |

#### バージョン更新

| ファイル | 変更 |
|---------|------|
| MainForm.cs | タイトル v0.4.9 |
| MovieConverter.csproj | バージョン 0.4.9.0 / v0.4.9 |
...
### Note
Windows Media Player COM (wmp.dll) を使ったプレビュー方式を検証実装しました。ログ行右側に「プレビュー方式」コンボボックスを追加し、標準プレビュー（WebView2）・代替プレビュー（静止画）・WMPプレビュー（検証）の3方式を切り替えて利用できます。WMP方式は検証目的の実装であり、長期前提での主方式としての採用は行いません。
...
- `LoadVideo` 時に初めて COM を初期化（遅延初期化）
- AxHost ラッパーで WMP OCX を WinForms パネルに埋め込み
- ポーリングタイマー（500ms）で再生位置・再生状態を取得
- 5秒タイムアウト検出（10ポーリング周期経過で VideoError 発火）
- COM 操作はすべて try-catch で保護し、失敗時は VideoError イベントで通知

**プレビュー方式の切り替え:**
- ログ行右側（「動作確認」ボタンの右）にコンボボックスを配置
- ファイルを選択するたびにコンボボックスで選択した方式で読み込む
- WMP 方式で初期化・読み込みに失敗した場合は自動的に代替プレビューへ切り替え

#### バージョン更新

| ファイル | 変更 |
|---------|------|
| MainForm.cs | タイトル v0.4.7 |
| MovieConverter.csproj | バージョン 0.4.7.0 / v0.4.7 |

---

## v0.4.6
### Title
動画簡易変換ツール v0.4.6 — v0.4系安定化・配布前整理

### Note
新機能の追加はありません。v0.4系の配布前整理として、プレビュー方式の説明整理、配布フォルダ構成の明確化、初回確認手順の整備、よくあるトラブルの一般職員向け/管理者向け整理、リリースチェックリスト更新、開発報告書のv0.4系到達点記録を行いました。
...
| ドキュメント | 変更内容 |
|-------------|---------|
| README.md | よくあるトラブルを一般職員向け・管理者向けに分離。v0.4.6 バージョン履歴追加 |
| manuals/user_manual.md | 代替プレビューセクション改善（可/不可の表・6ボタン記述・利用者向け文言整理）。フッターバージョン更新 |
| manuals/admin_manual.md | フォルダ構成に `logs/` 追加・DLL配置の注意強調。初回確認チェックリストをWebView2あり/なし端末に分けて整備。セクション12の代替プレビュー説明を拡充（v0.4.5の6ボタン記載・よくあるトラブル追加） |
| docs/release_checklist.md | v0.4.6 チェック項目追加（WebView2あり/なし・ffmpeg/ffprobe・DLL・日本語パス・空白パス）。バージョン更新 |
| docs/release-note.md | v0.4.6 エントリ追加 |
| development_report.md | v0.4.6 セクション追加（v0.4系到達点・WebView2なし対応方針・WPF不採用理由・代替プレビュー位置づけ・v0.5.0以降候補）。バージョン更新 |

#### バージョン更新（コード）

| ファイル | 変更 |
|---------|------|
| MainForm.cs | タイトル v0.4.6 |
| MovieConverter.csproj | バージョン 0.4.6.0 / v0.4.6 |

---

## v0.4.5
### Title
動画簡易変換ツール v0.4.5 — 代替プレビュー操作性改善

### Note
代替プレビューの操作性を大幅に改善しました。1秒単位のナビゲーションボタンを追加し、現在位置の時刻を大きく見やすく表示するようにしました。また、画像更新中の状態表示・時刻入力のエラー表示・ユーザー向けの説明文改善など、使いやすさを向上させました。

### 変更内容

#### 新機能・改善: 代替プレビューパネル（FallbackPreviewPanel）

**ナビゲーションボタンの追加（6ボタン体制）:**
...
**現在位置表示の改善:**
- 現在位置の時刻を Consolas 13pt Bold の大きなフォントで中央に常時表示（`_lblCurrentTime`）

**画像更新中の状態表示:**
- 時刻移動時に `⏳ 画像を更新中...` をステータスラベルに表示
- 画像生成完了後に自動でクリア
- 連続クリック時は前の生成処理を CancellationToken でキャンセルして重複を防止

**時刻入力の改善:**
- 対応形式: `hh:mm:ss`・`h:mm:ss`・`mm:ss`・`m:ss`・小数を含む秒数（例: `90`・`1.5`）
- 不正な形式入力時: テキストボックスをピンク背景で強調・エラーメッセージを3秒間表示
- 動画の長さを超えた時刻入力: 最大値（動画終端）に自動補正

**説明文の改善:**
- 「ffmpeg」「静止画生成」等の技術用語を表示せず、利用者向けの説明文に変更
- 説明エリアを 40px に拡大して視認性を向上

#### 変更ファイル

| ファイル | 変更内容 |
|---------|---------|
| FallbackPreviewPanel.cs | 全面再実装（6ボタン・大きな時刻表示・状態ラベル・時刻入力改善） |
| MainForm.cs | タイトル v0.4.5 更新 |
| MovieConverter.csproj | バージョン 0.4.5.0 / v0.4.5 に更新 |
| README.md | v0.4.5 バージョン履歴追加・代替プレビュー説明更新 |

---

## v0.4.4
### Title
動画簡易変換ツール v0.4.4 — 代替プレビュー安定化・仕様明確化

### Note
代替プレビュー（v0.4.3）の安定化確認と軽微修正を行いました。ファイル切り替え時にプレビュー表示が正しくリセットされない問題を修正し、標準プレビューと代替プレビューの仕様をドキュメントで明確化しました。

### 変更内容

#### バグ修正: ファイル切り替え時のプレビュー表示リセット

**問題:** 代替プレビューが使用された後に別のファイルを読み込んだとき、`LoadFile()` 内で `_fallbackPreviewActive = false` にリセットしていたが `_fallbackPreview.Visible = false` と `_player.PreviewControl.Visible = true` のリセットが漏れていた。

**影響:** WebView2 インストール済み環境で、ファイルAを読み込む → 代替プレビューが必要なファイルBを読み込む → 通常再生可能なファイルCを読み込む、の順で操作すると、ファイルCがWebView2で正常に再生できるにもかかわらず代替プレビューパネルが表示されたままになる可能性があった。

**修正:** `LoadFile()` 内、`_player.LoadVideo(filePath)` の前にプレビュー表示を標準プレイヤーへ楽観的にリセットする処理を追加。WebView2 エラーが発生した場合は `OnVideoPlayerError → ShowFallbackPreview()` で再び切り替わる。

#### ドキュメント: プレビュー方式の仕様明確化

| ドキュメント | 変更内容 |
|-------------|---------|
| README.md | プレビュー方式の比較表追加（標準＝動画再生・代替＝サムネイルのみ） |
| user_manual.md | ステップ1にプレビュー種類の説明を追加 |
| admin_manual.md | セクション12にプレビュー方式比較表を追加。WPF 注記を現状反映に更新 |
| test_scenarios.md | Section 21（代替プレビュー安定化確認シナリオ）追加 |
| release_checklist.md | v0.4.4 チェック項目追加 |

---

## v0.4.3
### Title
動画簡易変換ツール v0.4.3 — WebView2 未インストール環境向け代替プレビュー追加

### Note
WebView2 Runtime が未インストールの環境で、ffmpeg を使ったサムネイル表示によるカット位置確認ができるようになりました。

### 変更内容

#### 新機能: 代替プレビュー（FallbackPreviewPanel）

WebView2 Runtime が未インストールの環境でMP4ファイルを読み込んだとき、ffmpeg でサムネイル画像を生成してカット位置を確認できる代替プレビューパネルを表示する。

**代替プレビューの機能:**

| 機能 | 内容 |
|------|------|
| サムネイル表示 | 現在位置のフレームを `ffmpeg -frames:v 1` で生成して表示 |
| ナビゲーション | ◀◀10秒・◀5秒・5秒▶・▶▶10秒ボタンで位置移動 |
| 時刻直接入力 | テキストボックスに `HH:MM:SS` を入力して「移動」ボタンで指定位置へ移動 |
| 現在位置の同期 | 移動のたびに「現在位置を開始に設定」「現在位置を終了に設定」ボタンが更新される |
| 変換との連携 | 代替プレビューで指定した位置からそのまま変換実行できる |

**代替プレビューが表示される条件:**

- WebView2 Runtime が未インストールのため `InitFailed = true` のとき

**代替プレビューが表示されない条件（従来どおり）:**

- WebView2 が有効だが特定の MP4 が再生できない場合 → 事前変換ダイアログ
...
一時ファイルは `%TEMP%\MovieConverter_preview\` に保存し、アプリ終了時に削除する。
