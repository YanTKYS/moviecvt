## v0.1.3
### Title
動画簡易変換ツール v0.1.3 — ffmpeg DLL配置手順・トラブルシュート追記
...
実機確認で、`ffmpeg.exe` 単体だけを配置した場合に終了コード `-1073741515`（DLL不足）が返る事象を確認。配布元ZIPのDLL一式を `bin\ffmpeg\` フォルダに配置する必要があることを、各ドキュメントに追記した。

- `docs/ffmpeg_setup.md`: DLL配置が必須である旨を強調、フォルダ構成例にDLL一覧を追記、`-version` での動作確認手順を追加、終了コード -1073741515 のトラブルシュートを追加
- `manuals/admin_manual.md`: 配布パッケージ構成にDLLを明記、配置手順に `-version` 確認ステップを追加、障害対応に終了コード -1073741515 セクションを追加
- `README.md`: セットアップの ffmpeg 配置説明にDLL配置の注意・`-version` 確認を追加、バージョン履歴を追記
- `docs/test_scenarios.md`: DLL配置確認テスト・終了コード -1073741515 確認テスト（セクション10）を追加
- `docs/release_checklist.md`: DLL配置確認・`-version` 確認チェック項目を追加

#### ソースコードの変更なし

本バージョンはドキュメントのみの更新。アプリ本体・player.htmlに変更なし。
