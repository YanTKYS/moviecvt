...
v0.2.0 完了後、以下を v0.3.0 以降の候補として整理する。
...
| # | 変更対象 | 内容 |
|---|----------|------|
| 1 | `FfmpegRunner.cs` | `BuildFaststartOutputPath` を追加（`{baseName}_faststart_{timestamp}.mp4`）。`RunFaststartAsync` を追加（`-c copy -movflags +faststart` コマンド）。プロセス実行コアを `RunProcessAsync` として共通化し `RunAsync` と `RunFaststartAsync` から呼び出す形に整理 |
| 2 | `MainForm.cs` | `btnFaststart`（「MP4を整える」）を pnlFile に追加。`_activeOperationLabel` フィールドを追加し `ElapsedTimer_Tick` でタイマーラベルを動的切り替え。`BtnFaststart_Click`・`OnFaststartCompleted` を追加。`SetConvertingState` に `btnFaststart` の有効/無効制御を追加。`UpdateConvertButton` に `btnFaststart.Enabled` 管理を追加。タイトルを v0.2.0 に更新 |
...
| 判断事項 | 内容 |
|---------|------|
| 元ファイル変更なし | `{name}_faststart_{timestamp}.mp4` として常に別ファイルを作成。上書きしない |
| 自動実行なし | ユーザーがボタンを押したときのみ実行。ファイル読み込み時に大容量ファイルの自動コピーをしない |
| 再エンコードなし | `-c copy` を使用。画質劣化・サイズ変化なし（moov atom の再配置のみ） |
| 整備後の自動読み込み | 整備完了後に出力ファイルを `LoadFile()` でプレビューに自動読み込み。利用者がすぐ変換実行できる |
| ログ表現 | 「MP4を整える」「MP4構造の整備が完了しました」等、実装詳細（faststart）を露出しない表現を採用 |

#### 対応しなかった内容

| 事項 | 理由 |
|------|------|
| 変換出力への自動 faststart 化 | 変換処理は再エンコード済みでありmoov atom位置はffmpegデフォルト（末尾）になる場合がある。高速カット出力も同様。v0.3.0以降の検討事項 |
| プレビュー不可時の自動整備提案 | 自動で大容量ファイルのコピー処理を開始しない方針とした。ユーザーが明示的に選択する形を維持 |
