...
| アプリバージョン | v0.4.9 |
| OS | （確認後に記入） |
| 端末条件 | （確認後に記入：WebView2 あり/なし・ffmpeg 配置状況など） |
| テスト動画の形式 | （確認後に記入：コーデック・解像度・長さ・容量など） |
| 確認日時 | （確認後に記入） |
...
|----------|------|------|
| アプリが起動する | — | |
| 起動直後にWMP COMが初期化されない | — | ログに `[WMP]` が出ないこと |
| 標準プレビューが従来どおり利用できる | — | |
| 代替プレビューが従来どおり利用できる | — | |
...
| 確認項目 | 結果 | 備考 |
|----------|------|------|
| WMPプレビュー（検証）を選択してMP4を読み込める | — | |
| WMPプレビュー領域に動画が表示される | — | |
| 動画時間が取得されシークバーが有効になる | — | |
| 音量スライダーが有効になる | — | |
...
| 確認項目 | 結果 | 備考 |
|----------|------|------|
| 標準プレビューからWMPプレビューへ切り替えられる | — | |
| WMPプレビューから代替プレビューへ切り替えられる | — | |
| 切り替え後も動画読み込みが破綻しない | — | |
| WMP失敗時に代替プレビューへ自動切替される | — | |

#### 7. 異常系確認

| 確認項目 | 結果 | 備考 |
|----------|------|------|
| WMPで再生できないMP4を読み込んだ場合に代替プレビューへ切り替わる | — | |
| ファイルを切り替えても正常に再読み込みできる | — | |
| 読み込み中に別ファイルを選択しても破綻しない | — | |
| 変換中にキャンセルしても正常に終了できる | — | |
| WMPプレビュー利用後にアプリを終了しても残プロセスがない | — | |
...
| 音量調整 | ○ | — |
| 安定性 | — | — |
| 導入手順 | WebView2 Runtime が必要 | 追加インストール不要 |
...
以下の観点で判断する:
- 本番環境で基本操作（再生・シーク・音量・変換連携）が安定して動作したか
- 既存の標準プレビュー・代替プレビューと比較して導入価値があるか
- 追加の実装・修正が必要な課題があるか

| 判断 | 内容 |
|------|------|
| WMPプレビューの実用性評価 | （確認後に記入） |
| 今後の扱い | （確認後に記入）検証継続 / WebView2なし環境の候補に格上げ / 削除 |
| 根拠 | （確認後に記入） |
