...
| v0.4.9更新日 | 2026-05-26 |
| v0.4.10更新日 | 2026-05-26 |
| 参照ガイド | reference/guide_context.md（同梱方式） |
| GitHub Pages | 403エラーにより参照不可 — guide_context.md で代替 |

---

## v0.4.9 WMPプレビュー本番環境実機確認

### 確認目的

v0.4.7 で追加した「WMPプレビュー（検証）」が本番 Windows 環境で実用可能かを確認する。
確認結果をもとに、今後 WMPプレビューをどう扱うか（検証継続 / WebView2 なし環境の候補に格上げ / 削除）を判断する。
...
| アプリバージョン | v0.4.9 |
| OS | （不明） |
| 端末条件 | WebView2 未インストール（Dll was not found）・ffmpeg 配置済み（変換動作確認ずみ） |
| テスト動画の形式 | MP4・379.3 MB・長さ 01:28:47 |
| 確認日時 | 2026-05-26 |

### 確認結果

> 凡例: ○ 正常動作 / △ 部分的に動作 / × 動作しない / — 未確認
...
|----------|------|------|
| アプリが起動する | ○ | |
| 起動直後にWMP COMが初期化されない | ○ | 起動ログに `[WMP]` なし |
| 標準プレビューが従来どおり利用できる | × | WebView2 Runtime 未インストールのため利用不可 |
| 代替プレビューが従来どおり利用できる | ○ | WebView2なし環境で代替プレビューへ正常切り替え |

#### 2. WMPプレビュー読み込み確認

| 確認項目 | 結果 | 備考 |
|----------|------|------|
| WMPプレビュー（検証）を選択してMP4を読み込める | × | 初期化失敗（後述のバグによる） |
| WMPプレビュー領域に動画が表示される | — | 初期化失敗のため未確認 |
| 動画時間が取得されシークバーが有効になる | — | 初期化失敗のため未確認 |
| 音量スライダーが有効になる | — | 初期化失敗のため未確認 |
...
| 再生できる | — | 初期化失敗のため未確認 |
| 一時停止できる | — | 初期化失敗のため未確認 |
| 5秒戻る・5秒進むが動作する | — | 初期化失敗のため未確認 |
| シークバーで位置移動できる | — | 初期化失敗のため未確認 |
| 音量変更できる | — | 初期化失敗のため未確認 |
| 消音できる | — | 初期化失敗のため未確認 |
...
| 確認項目 | 結果 | 備考 |
|----------|------|------|
| 現在位置を開始位置に設定できる | — | 初期化失敗のため未確認 |
| 現在位置を終了位置に設定できる | — | 初期化失敗のため未確認 |
| 指定範囲が正しく表示される | — | 初期化失敗のため未確認 |
| 開始・終了位置の手入力補正ができる | — | 初期化失敗のため未確認 |

#### 5. 変換連携確認

| 確認項目 | 結果 | 備考 |
|----------|------|------|
| WMPで指定した範囲を高速カットできる | — | 初期化失敗のため未確認 |
| WMPで指定した範囲を速度優先で変換できる | — | 初期化失敗のため未確認 |
| WMPで指定した範囲を標準で変換できる | — | 初期化失敗のため未確認 |
| 全体変換ができる | — | 初期化失敗のため未確認 |
| 変換後ファイルが再生できる | — | 初期化失敗のため未確認 |

#### 6. 方式切替確認

| 確認項目 | 結果 | 備考 |
|----------|------|------|
| 標準プレビューからWMPプレビューへ切り替えられる | — | 標準プレビュー自体が利用不可のため未確認 |
| WMPプレビューから代替プレビューへ切り替えられる | — | 初期化失敗のため未確認 |
| 切り替え後も動画読み込みが破綻しない | △ | WMP失敗後の再読み込みは正常（後述） |
| WMP失敗時に代替プレビューへ自動切替される | ○ | 代替プレビューへ正常切り替え・ffprobe で動画時間取得・変換可能状態に遷移 |

#### 7. 異常系確認

| 確認項目 | 結果 | 備考 |
|----------|------|------|
| WMPで再生できないMP4を読み込んだ場合に代替プレビューへ切り替わる | ○ | 初期化失敗トリガーでも代替プレビューへ切り替わる |
| ファイルを切り替えても正常に再読み込みできる | — | 未確認 |
| 読み込み中に別ファイルを選択しても破綻しない | — | 未確認 |
| 変換中にキャンセルしても正常に終了できる | — | 未確認 |
| WMPプレビュー利用後にアプリを終了しても残プロセスがない | — | 未確認 |

### 標準プレビューとの比較

| 比較項目 | 標準プレビュー（WebView2） | WMPプレビュー（検証） |
|----------|--------------------------|----------------------|
| 対応環境 | WebView2 Runtime インストール済み端末 | Windows 10/11（wmp.dll 標準インストール） |
| 再生品質 | — | — （初期化失敗のため確認不可） |
| シーク精度 | — | — （初期化失敗のため確認不可） |
| 音量調整 | ○ | — （初期化失敗のため確認不可） |
| 安定性 | — | — （初期化失敗のため確認不可） |
| 導入手順 | WebView2 Runtime が必要 | 追加インストール不要（WMP利用可能環境に限る） |

### 失敗した操作・確認できた制約

- **WMPプレビューの初期化が必ず失敗する**: エラーメッセージ「WMP インターフェースの取得に失敗しました」が発生し、代替プレビューへ自動切り替わる
- **原因（実装バグ）**: `EnsureInitialized()` で `WmpAxHost` を `Visible = false` のまま `Controls.Add` していたため、WinForms がハンドルを作成せず、`AxHost` の OCX 初期化（`CoCreateInstance`）が実行されなかった。`GetOcx()` は常に null を返した
- **ログ確認**: コンボボックスで WMP を再度選択すると自動再読み込みが走り、2回目は `_initFailed = true` による早期リターンに切り替わった（ログ: 「WMPプレビューは初期化に失敗しています」）

### 今後の扱いに関する暫定判断

| 判断 | 内容 |
|------|------|
| WMPプレビューの実用性評価 | 評価不可（Visible=false バグにより全 WMP 動作確認が未実施） |
| 今後の扱い | v0.4.10 でバグ修正後に再テスト |
| 根拠 | `Visible = false` の1行修正で初期化が通るはずであるため、バグ修正後の動作確認が先決 |

### 残課題

- **v0.4.10 対応済み**: `WmpVideoPlayer.EnsureInitialized()` の `AxHost Visible=false` バグ修正
- **再テスト必要**: v0.4.10 ビルドで WMP 初期化が成功するか確認する
- **WMP 利用可否の確認**: テスト環境で WMP COM（CLSID `6BF52A4F-394A-11D3-B153-00C04F79FAA6`）が登録されているか確認する（WebView2 未インストール環境でも WMP 自体は利用可能な場合が多いが、今回のテスト端末では確認できていない）

---

## v0.4.10 WMPプレビュー AxHost 初期化バグ修正

### 背景・目的

v0.4.9 実機確認で WMPプレビューの初期化が必ず失敗することが判明した。原因調査の結果、`WmpVideoPlayer.EnsureInitialized()` に実装バグ（`AxHost` を `Visible = false` のまま親に追加していた）があることを特定し、修正した。

### バグ詳細

| 項目 | 内容 |
|------|------|
| 発生メソッド | `WmpVideoPlayer.EnsureInitialized()` |
| 症状 | `GetOcxObject()` が常に null を返し「WMP インターフェースの取得に失敗しました」で失敗 |
| 根本原因 | `Visible = false` のコントロールを `Controls.Add` しても WinForms はハンドルを作成しない。AxHost はハンドル作成時に `CoCreateInstance` で OCX を初期化するため、Visible=false では初期化が一切実行されない |
| 影響範囲 | WMP が利用可能な環境でも WMPプレビューが必ず失敗していた |

### 修正内容

| ファイル | 変更 |
|---------|------|
| `WmpVideoPlayer.cs` | `EnsureInitialized()`: `WmpAxHost { Visible = false }` → `WmpAxHost {}` (デフォルト Visible=true) に変更。`_container.Handle` を先に取得してコンテナのハンドルを確保。`CreateControl()` 呼び出しを削除（Controls.Add 時に親ハンドルがあれば自動作成のため不要）。`_axHost.Visible = true` 代入を削除。エラーメッセージを詳細化 |

### 修正後の期待動作

- WMP COM（CLSID `6BF52A4F-394A-11D3-B153-00C04F79FAA6`）が登録されている環境では `GetOcxObject()` が COM オブジェクトを返し、WMPプレビューが起動する
- WMP が登録されていない環境では「WMP OCX の取得に失敗しました。Windows Media Player がインストールされていないか、オプション機能として無効になっている可能性があります。」のエラーメッセージとともに代替プレビューへフォールバックする

### 実機確認結果（v0.4.10）

同一端末（WebView2 未インストール・庁内端末）で v0.4.10 を実行した。

| 確認項目 | 結果 | 備考 |
|----------|------|------|
| Visible=false バグ修正の効果 | ○ | null エラーではなく `COMException` が発生し、WMP の有無が正しく判定される |
| WMP COM の利用可否 | × | `REGDB_E_CLASSNOTREG (0x80040154)` — CLSID がレジストリ未登録 |
| エラーメッセージの改善 | ○ | 「クラスが登録されていません (0x80040154)」と原因が明示される |
| 代替プレビューへの自動切替 | ○ | エラー後に代替プレビューへ正常遷移、変換可能状態に移行 |

**`REGDB_E_CLASSNOTREG` の意味:** Windows Media Player OCX が COM レジストリに存在しない。発生条件として以下が想定される。

| 条件 | 説明 |
|------|------|
| Windows N エディション | メディア機能パック未インストール |
| Windows のオプション機能で WMP が無効化 | 組織ポリシーまたは手動で無効化 |
| Windows 11 で WMP が削除された端末 | 新規インストール後に WMP を除外 |
| 仮想・端末管理環境での制限 | 庁内端末管理ポリシーによる制限 |

### 今後の WMPプレビュー機能の扱い判断

| 判断項目 | 内容 |
|----------|------|
| テスト端末での WMP 利用可否 | 利用不可（REGDB_E_CLASSNOTREG） |
| テスト端末の代表性 | 庁内標準端末（WebView2 未インストール）→ 代替プレビューが必要な層と重なる |
| WMP が使えない端末で WMP が使える可能性 | 低い（WebView2 もない端末は往々にして WMP も無効化されている） |
| コードとしての完成度 | v0.4.10 修正により動作は正常（WMP 利用可能環境では機能するはず） |
| 採用判断 | **削除候補**: 代替プレビューが必要な環境（WebView2 なし）では WMP も利用できないケースが多く、WMPプレビューが実際に活用される場面が想定しにくい |
| 削除時期 | 即時削除は不要。検証機能として残しつつ、v0.5.0 整理時に削除を検討する |
...
v0.4.6 で v0.4 系の安定化・配布前整理が完了した。v0.4.7 では WebView2 Runtime 未インストール環境でも動画再生によるプレビューができるよう、Windows Media Player COM (wmp.dll) を使ったプレビュー方式を検証実装した。

### WMP COM 検証実装の判断

| 観点 | 内容 |
|------|------|
| 採用目的 | WebView2 なし端末でも再生・シーク・音量調整を試すための検証 |
| 採用範囲 | 「WMPプレビュー（検証）」コンボボックス選択時のみ有効 |
| 長期採用 | しない。WMP COM は Microsoft が今後推奨しない方針のため |
| 失敗時の扱い | VideoError イベント → 代替プレビューへ自動切り替え |
| 起動への影響 | なし。InitializeAsync はノーオペレーション（LoadVideo 時に遅延初期化） |
...
WMP 方式が実機で安定動作するか確認した上で、v0.5.0 以降の方針を決定する。

- **安定する場合**: 代替プレビュー（静止画）を WMP プレビューに置き換えるか、共存させる
- **安定しない場合**: WMP 方式を削除し、代替プレビューに一本化する（その場合 WmpVideoPlayer.cs は削除対象）
...
v0.4.5 で代替プレビューの操作性改善が完了し、v0.4系の機能実装は一区切りとなった。v0.4.6 では新機能追加は行わず、配布前の最終整理として以下を実施した。

- プレビュー方式（標準・代替）の説明を利用者・管理者向けに整理
- 配布フォルダ構成の最終確認・明文化（`logs/` フォルダ追加、DLL配置の注意強調）
- 初回確認手順をWebView2あり/なし端末に分けて整備
- よくあるトラブルを一般職員向け・管理者向けに分離
- リリースチェックリストにv0.4.6確認項目を追加
- 開発報告書にv0.4系の到達点を記録
...
| # | 変更対象 | 内容 |
|---|----------|------|
| 1 | MainForm.cs | タイトル v0.4.6 更新 |
| 2 | MovieConverter.csproj | バージョン 0.4.6.0 / v0.4.6 に更新 |
| 3 | README.md | よくあるトラブルを一般職員向け・管理者向けに分離。v0.4.6 バージョン履歴追加 |
| 4 | manuals/user_manual.md | 代替プレビューセクション改善（可/不可の表・6ボタン記述・利用者向け文言整理）。フッターバージョン更新 |
| 5 | manuals/admin_manual.md | フォルダ構成に `logs/` 追加。初回確認チェックリストをWebView2あり/なし端末に分離。セクション12を拡充（v0.4.5の6ボタン・よくあるトラブル追加） |
| 6 | docs/release_checklist.md | v0.4.6 チェック項目追加（WebView2あり/なし・DLL・日本語パス等）。バージョン更新 |
| 7 | docs/release-note.md | v0.4.6 リリースノート追加 |

### v0.4系の到達点

v0.4.0〜v0.4.6 で以下を実現した。

| バージョン | 内容 |
|-----------|------|
| v0.4.0 | 変換結果の詳細表示・エラー診断改善・ログ保存 |
| v0.4.1 | WebView2 未インストール時のフリーズ修正・プレビュー不可モード |
| v0.4.2 | 環境診断機能・起動時ログ強化 |
| v0.4.3 | 代替プレビュー追加（ffmpeg サムネイル方式） |
| v0.4.4 | 代替プレビュー安定化（ファイル切替時の表示リセット修正） |
| v0.4.5 | 代替プレビュー操作性改善（6ボタン・大きな時刻表示・画像更新中表示・時刻入力改善） |
| v0.4.6 | ドキュメント整理・配布前最終確認 |

### WebView2 なし環境への対応方針

WebView2 Runtime が未インストールの端末では以下の方針を採用している。

1. **フリーズしない**: v0.4.1 で WebView2 初期化失敗を非同期で検知し、UIをフリーズさせない
2. **代替プレビューへ自動切替**: v0.4.3 で FallbackPreviewPanel を実装。ffmpeg によるサムネイル表示でカット位置確認を可能にした
3. **変換は必ず実行できる**: 代替プレビューでも開始・終了位置指定と変換実行は可能
4. **WebView2 なしを前提とした動作**: ffmpeg 未配置時でも代替プレビューパネルは表示され、テキスト入力での位置指定が可能

### WPF MediaElement を再採用しなかった理由

v0.3.1 で WPF MediaElement による代替プレビューを実装したが、`<UseWpf>true</UseWpf>` 追加により `DllNotFoundException` が発生し起動クラッシュした（v0.3.2 で即時ロールバック）。

代替プレビューとして WPF MediaElement を再採用しなかった理由:

| 理由 | 詳細 |
|------|------|
| 起動クラッシュの教訓 | UseWpf=true は起動クラッシュの直接原因。閉域・低スペック環境での配布リスクが高い |
| WinForms のみで実装可能 | ffmpeg + PictureBox の方式であれば既存の WinForms 構成で実現できる |
| ffmpeg の再利用 | サムネイル生成に既存の ffmpeg.exe を再利用できるため、追加コンポーネントが不要 |
| 動画再生不要 | 代替プレビューの目的はカット位置の確認であり、動画再生は必須ではない |

### 代替プレビューの位置づけ

代替プレビューは「WebView2 なし端末でも最低限カット位置を確認して変換できる」ことを目的とした機能であり、標準プレビューの完全な代替ではない。

- **標準プレビュー**: WebView2 による動画再生。再生・シーク・音量調整が可能
- **代替プレビュー**: ffmpeg による指定時刻のサムネイル表示。動画再生は不可

WebView2 がある端末では常に標準プレビューを使用し、代替プレビューは自動的に表示されない。

### v0.5.0 以降の候補

現時点での未対応事項・検討候補（優先順位は未確定）:

| 候補 | 概要 | 備考 |
|------|------|------|
| WebView2 なし端末での動画再生 | LibVLCSharp / Windows Media Player COM 等の採用検討 | 配布パッケージの複雑化・ライセンス管理が課題 |
| MP4以外の形式への対応 | AVI・MOV・MKV 等の入力対応 | ffmpeg が対応しているが、UI・テスト範囲が大幅拡大 |
| 複数区間のカット | 一つの動画から複数箇所を切り出す | UI設計の大幅変更が必要 |
| バッチ変換 | 複数ファイルの一括変換 | ユースケースの整理が必要 |
| ネットワークドライブへの安定した出力 | 書き込み失敗の診断改善 | 現状でも動作するが、エラーメッセージ改善の余地あり |
...
v0.4.3 で追加した代替プレビューは10秒・5秒のナビゲーションボタンのみで、1秒単位の微調整ができなかった。また現在位置の時刻表示が小さく視認しにくく、画像生成中の状態が不明確で、時刻入力の対応形式も限られていた。v0.4.5 では代替プレビューの操作性を全面改善した。

### 実施した変更

| # | 変更対象 | 内容 |
|---|----------|------|
| 1 | FallbackPreviewPanel.cs | 全面再実装。6ボタン（1秒戻る/進む追加）・大きな現在位置表示・画像更新中ステータス・時刻入力改善・説明文改善 |
| 2 | MainForm.cs | タイトル v0.4.5 更新 |
| 3 | MovieConverter.csproj | バージョン 0.4.5.0 / v0.4.5 に更新 |
| 4 | README.md | v0.4.5 バージョン履歴追加・代替プレビュー説明更新 |
| 5 | manuals/user_manual.md | 代替プレビュー説明に1秒ボタンの記述追加・バージョン更新 |
| 6 | manuals/admin_manual.md | バージョン更新 |
| 7 | docs/test_scenarios.md | Section 22 追加（v0.4.5 代替プレビュー操作性確認シナリオ） |
| 8 | docs/release_checklist.md | v0.4.5 チェック項目追加 |
| 9 | docs/release-note.md | v0.4.5 リリースノート追加 |

### 設計判断

| 判断 | 理由 |
|------|------|
| 1秒ボタンを既存の10秒・5秒ボタンと同列に追加 | 動画の細かい位置合わせに必要。ボタン数は6つで横幅に収まる範囲 |
| 現在位置表示を独立したラベルとして中央に配置 | 6ボタンの中央の空白スペースを活用。Consolas フォントで読みやすい |
| CancellationToken パターンを維持 | 連続クリック時の重複処理防止は v0.4.3 の設計を踏襲。新たな複雑さなし |
| エラー表示を3秒後に自動クリア | ユーザーが操作を止めずに次の入力を試せる。エラーメッセージが長く残らない |
| TryParseTime で複数形式対応 | `hh:mm:ss`・`h:mm:ss`・`mm:ss`・`m:ss`・秒数の5パターン。TimeSpan.TryParseExact と double.TryParse の組み合わせ |
